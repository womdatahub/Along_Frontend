...
import { callApi, paymentApiStr } from "@/lib";
import { toast } from "sonner";

type AdminType = {
    actions: {
        createRideCostSettings: () => Promise<void>;
    ...
    devtools(
        persist(
            (set) => ({
                ...initialState,
                actions: {
                    createRideCostSettings: async () => {
                        const { data, error } = await callApi(paymentApiStr('/cost/settings'), {})
                        if (error) {
