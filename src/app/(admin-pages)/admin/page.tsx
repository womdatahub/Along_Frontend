            ...
            <CardContent className="flex flex-col gap-6 px-0">
              {queues.map((q) => (
                <div key={q.text} className="flex gap-4 items-center">
                  <div
                    className={cn(
                      "w-[5.5vw] flex justify-center items-center py-0.5 font-medium rounded-full",
                      `${q.badgeColor} ${q.textColor}`
                    )}
                  >
                    {q.title}
                  </div>
            ...
            <CardContent className="flex flex-col px-0">
              {liveOperations.map((live, i) => (
                <div
                  ...
                  key={i}
                >
                  <div className="flex flex-col gap-1 font-heebo w-1/4">
                    <p className="text-[#768B8F] text-[8px]">Type</p>
                    <p className="text-black text-[9px]">{live.type}</p>
                  </div>
                  <div className="flex flex-col gap-1 font-heebo w-1/4">
                    <p className="text-[#768B8F] text-[8px]">Customer</p>
                    <p className="text-black text-[9px]">{live.customer}</p>
                  </div>
                  <div className="flex flex-col gap-1 font-heebo w-1/4">
                    <p className="text-[#768B8F] text-[8px]">Driver</p>
                    <p className="text-black text-[9px]">{live.driver}</p>
                  </div>
                </div>
              ))}
            </CardContent>
